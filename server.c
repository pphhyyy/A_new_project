...
//用一个 SockInfo 的数组来存储 所有子线程的 SockInfo 
...
int main()
{
    ...
    for (int i = 0 ; i < max ; i++)
    {
        ...
        infos[i].fd = -1; // 数组中某个位置的文件描述符是 -1 说明这个位置是空的,下面accept 以后 只要判断是否有-1 就可以把数据填入这个位置
    }
    ...
    //向线程池中放任务 
    ThreadPoolAdd(pool , acceptConnect , NULL)
    return NULL;
}
...
void acceptConnect(void * arg)
{
    ...
    close(fd); // 关闭监听用的文件描述符 , 这里通信的文件描述符cfd在子线程中被使用,不需要主线程关闭
